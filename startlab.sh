...
export AWS_SIZE_2XLARGE="2xlarge_${AWSLS_VM_SIZE_SUFFIX}"
...
open-vm-standard-network-port $VM_PREFIX-devsecops
...
for vm in rancher devsecops; do
  ...
  scp $SSH_OPTS -i mylab.key -r setup/$vm/*  ec2-user@$VM_IP:~/
  scp $SSH_OPTS -i mylab.key mylab_vm_list.txt ec2-user@$VM_IP:~
done 
